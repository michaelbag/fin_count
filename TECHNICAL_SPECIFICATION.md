...
- `rate` (DecimalField, max_digits=15, decimal_places=6) - Курс обмена
...
- [ ] CurrencyRate (Курс валют) - конкретная модель, наследуется от BaseReference, с историей курсов по датам
...
### 8.5. Бизнес-логика
- [ ] Генерация номеров документов (автоматическая генерация при сохранении, если номер пустой)
- [ ] Настройка глобального префикса номеров документов (модель настроек или settings.py)
