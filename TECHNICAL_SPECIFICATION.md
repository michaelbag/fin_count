...
- Поддержка частичного и полного возврата неиспользованных средств непосредственно через отчет с возможностью закрытия выдачи
- Контроль допустимых сумм возврата и доплаты (включая ручные значения): суммы не могут превышать доступный остаток или создавать отрицательный баланс по выдаче
...
- Проверка суммы возврата на превышение текущего остатка по выбранной выдаче с учетом уже подтвержденных авансовых отчетов и ранее оформленных возвратов
...
- Закрытие может быть инициировано как подтвержденным авансовым отчетом, так и документом возврата при соблюдении контрольных проверок по остаткам
...
- `validate_balance_limits()` - Проверить, что суммы возврата и доплаты не превышают доступные остатки по связанной выдаче с учетом ранее подтвержденных документов
...
- При подтверждении отчета (status='confirmed'):
  ...
  - Суммы возврата в отчете ограничиваются остатком по выдаче с учетом уже проведенных возвратов и ранее подтвержденных отчетов
  - Доплаты ограничиваются неотчитанным остатком и не допускают отрицательного баланса
...
**Методы:**
- `clean()` - Проверка суммы возврата на превышение текущего остатка по выдаче с учетом подтвержденных авансовых отчетов и ранее проведенных возвратов
...
1. При установке статуса 'confirmed' (Подтвержден):
   ...
   - Создается операция возврата (если есть возврат)
     - Сумма возврата ограничивается доступным остатком по выдаче с учетом ранее подтвержденных авансовых отчетов и оформленных возвратов
     - Поддерживаются частичные и полные возвраты; при сумме, равной остаткам, и установленном флаге `close_advance_payment` выдача закрывается автоматически
   - Создается операция доплаты (если есть доплата)
     - Доплата не может превышать превышение фактических расходов над суммой выдачи
   - Если `close_advance_payment=True`, подотчетные средства закрываются
   - Если `close_advance_payment=False` и есть доплата, создается новая выдача подотчетных средств
...
- При использовании ручных сумм выполняется проверка лимитов, исключающая превышение доступного остатка или формирование отрицательного баланса по выдаче
...
2. При проведении документа выполняется проверка на то, что сумма возврата (с учетом возвратов из подтвержденных авансовых отчетов и ранее проведенных документов возврата) не превышает текущий остаток по выбранной выдаче
...
- Возвраты, оформленные через авансовые отчеты, учитываются при расчете доступного остатка для документа возврата
...
- Проверки допустимых сумм в документах авансового отчета и возврата используют эти расчеты, предотвращая превышение остатка
