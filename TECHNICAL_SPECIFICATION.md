...
**Вопрос: Закрытие (подтверждение) сотрудником ранее выданных денег происходит строго по той же статье расходов, по которой деньги выдавались?**

- **Требование:** При закрытии (подтверждении) сотрудником ранее выданных подотчетных средств в авансовом отчете, строки отчета должны использовать ту же статью расходов, по которой была произведена первоначальная выдача денег
- **Валидация:** Система должна проверять соответствие статей расходов в строках авансового отчета статье расходов, указанной при выдаче подотчетных средств
- **Исключения:** Возможность использования других статей расходов (если требуется) должна быть явно разрешена в настройках системы или подтверждена администратором
...
- Расчет на основе выданных сумм минус суммы подтвержденных отчетов
...
**Реализация:**
- При создании строк авансового отчета система должна проверять соответствие статей расходов в строках отчета статье расходов, указанной при выдаче подотчетных средств (AdvancePayment)
- Валидация должна выполняться на уровне формы и модели при сохранении авансового отчета
- Если в строке отчета указана статья расходов, отличная от статьи расходов при выдаче, система должна:
  - Выдать предупреждение пользователю
  - Запросить подтверждение или разрешение администратора (в зависимости от настроек системы)
  - Либо заблокировать сохранение отчета до исправления статьи расходов

**Исключения:**
- Возможность использования других статей расходов может быть разрешена в настройках системы
- Администратор может иметь право подтверждать авансовые отчеты с отличными статьями расходов
- В случае изменения статьи расходов должна быть возможность указать причину изменения
...
- **ВАЖНО: Закрытие (подтверждение) сотрудником ранее выданных денег должно происходить строго по той же статье расходов, по которой деньги выдавались. Система должна выполнять валидацию соответствия статей расходов при создании авансового отчета.**
